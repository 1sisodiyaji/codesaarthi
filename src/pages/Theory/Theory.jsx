  ...
  const handleBookmarkClick = async (id) => {
    try {
      const response = await axios.post(
        `${config.BASE_URL}/Admin/BookMark/${id}`,
        { userId }
      );
      ...
      toast.error("please login to bookmark!", { theme: "dark" });
    }
  };
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-between">
                    <Link to={`/Theory/${item.slug}`}>
                      <button className="text-white dark:text-white bg-violet-500 hover:bg-violet-600 rounded-lg md:text-sm text-xs md:px-5 md:py-2.5 p-2 text-center">
                        Start Reading{" "}
                    ...
                    <div className="space-x-2">
                      <button
                        className={`text-black dark:text-white ${bookmarkedItems[item._id]
                            ? "bg-green-500"
                            : "bg-orange-500"
                      ...
                      >
                        <i
                          className={`${bookmarkedItems[item._id]
                              ? "fi fi-sr-bookmark"
                              : "fi fi-br-bookmark"
                            }`}
                        ></i>
